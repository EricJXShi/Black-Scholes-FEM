...
A = zeros(n_nodes, n_nodes); % Initialize Global 'Stiffness' Matrix
...
for i = 1:n_blocks
    ...
    ket2 = (1/dtao + theta*m); % Local 'Stiffness' Matrix for the next time step
...
for t = 2:n_time
    B = zeros(n_nodes, 1); % Initialize the 'Forcing' Vector (RS of equation)
    for i = 1:n_blocks
        S1 = S(i); S2 = S(i+1); % Local node stock price coordinates
        enodes = conn(i, :); % Element to node connectivity for local to global mapping
        m = kD\(2/dS*((sigma^2-r)*(1/2*S1*kB1 + 1/2*S2*kB2) + 1/8*...
            sigma^2*(S1^2*kE1 + 2*S1*S2*kE2 + S2^2*kE3) + r*kD)); % [m] in the hand calculation
        ket1 = (1/dtao - (1-theta)*m); % Local 'Stiffness' Matrix for the current time step
        B(enodes) = B(enodes) + ket1*V(enodes); % Assembly of the global forcing vector
    end
    B_F = B(free_dofs);
    V_E = V_total(fixed_dofs, t);
    V_F = A_F\(B_F - A_FE*V_E);
    V_total(2:end-1, t) = V_F;
    V = V_total(:, t);
end
V_total;
