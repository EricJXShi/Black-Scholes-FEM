...
figure(2)
fplot(SP, Exact_Solution, [Stock_Price/2, Stock_Price*2], 'LineWidth', 2)
hold on
plot(S, V_total(:, end), 'LineWidth', 2)
hold off
xlabel('Stock Price ($)')
ylabel('Option Price ($)')
...
%% Print Results
fprintf('The current option price using Finite Element Method is $%.2f\n', Option_Price)
fprintf('The current option price using Black-Scholes Equation is $%.2f', subs(Exact_Solution, Stock_Price))