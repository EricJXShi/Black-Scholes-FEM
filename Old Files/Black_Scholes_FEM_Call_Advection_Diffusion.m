...
T = 30/365.25; % Years Until Expiration
...
n_free_blocks = n_blocks - 2; % Number of Free Elements
n_free_nodes = n_nodes - 2; % Number of Free Nodes
...
V = V(2:end-1); % Remove the fixed DOFs from V for FEM

%% Define Connectivity of Global Free DOFs
conn = zeros(n_free_blocks, 2); % Initialize element to node connectivity
for i = 1:n_free_blocks
...
A = zeros(n_free_nodes, n_free_nodes); % Free Global 'Stiffness' Matrix
B = zeros(n_free_nodes, 1); % Forcing Vector

%% Local Matrix Assembly
theta = 1/2; % Central difference method
for i = 1:n_free_blocks
    %% Element 'Stiffness' Matrix Assembly (Theta Differencing)
    S1 = S(i+1); S2 = S(i+2);
    enodes = conn(i, :); % Element to node connectivity for local to global mapping
    m = kD\(2/dS*((sigma^2-r)*(1/2*S1*kB1 + 1/2*S2*kB2) + 1/8*...
        sigma^2*(S1^2*kE1 + S1*S2*kE2 + S2^2*kE3) + r*kD)); % [m] in the hand calculation
