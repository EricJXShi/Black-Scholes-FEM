...
for i = 1:n_time
    x(2:end, i) = log(S(2:end)) + (r-1/2*sigma^2)*tao(i); % Space Vector, where x is a function of stock price and tao
end

dx = zeros(n_blocks, n_time);
for j = 1:n_time
    for i = 1:n_blocks
...
dndx = 2./dx; % Relationship between dS and dn in the form of dn/dS
dN1dn = diff(N1n); dN2dn = diff(N2n); % Derivative of shape functions wrt n
dN1dx = dN1dn*dndx; dN2dx = dN2dn*dndx; % Derivative of shape functions wrt S
...
for j = 1:n_time
    ...
    for i = 1:n_blocks
        kC = [k11eC(i, j), k12eC(i, j); k21eC(i, j), k22eC(i, j)];
