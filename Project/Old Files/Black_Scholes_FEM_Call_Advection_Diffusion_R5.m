...
B = zeros(n_nodes, 1); % Initialize Global 'Stiffness' Matrix for the current time step
...
for i = 1:n_blocks
    ...
    ket1 = alpha_coeff\(1/dtao - (1-theta)*beta)*V(enodes); % Local 'Stiffness' Matrix for the current time step
    ket2 = alpha_coeff\(1/dtao + theta*beta); % Local 'Stiffness' Matrix for the next time step
    A(enodes, enodes) = A(enodes, enodes) + ket2; % Assembly of the global 'stiffness' matrix
    B(enodes) = B(enodes) + ket1; % Assembly of the global forcing vector
end
...
for t = 2:n_time
    ...
    V_F2 = A_F\(B_F - A_FE*V_E2); % Free DOF Option pricing at next time step
